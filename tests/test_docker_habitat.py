...
def test_docker_habitat_execute_failed_command(docker_habitat):
    ...
    exit_code, output = docker_habitat.execute("exit 1")
...
def test_docker_habitat_volume_mount(docker_habitat, temp_workspace):
    """Test czy volume jest zamontowany poprawnie."""
    # Utwórz plik w workspace
    test_file = Path(temp_workspace) / "test_volume.txt"
...
def test_docker_habitat_file_creation_in_container(docker_habitat):
    ...
    exit_code, output = docker_habitat.execute(
        "echo 'Created in container' > /workspace/container_file.txt"
    )
    assert exit_code == 0
...
def test_docker_habitat_execute_with_stderr(docker_habitat):
    ...
    exit_code, output = docker_habitat.execute("python -c \"import sys; sys.stderr.write('Error message\\n')\"")
...
def test_docker_habitat_python_script_execution(docker_habitat, temp_workspace):
    """Test wykonania skryptu Python z pliku."""
    # Utwórz skrypt Python
...
def test_docker_habitat_python_script_with_error(docker_habitat, temp_workspace):
    """Test wykonania skryptu Python z błędem."""
    # Utwórz skrypt Python z błędem
    script_path = Path(temp_workspace) / "error_script.py"
    script_content = """
print("Before error")
...
@pytest.mark.skipif(
    ...
    reason="Docker nie jest dostępny"
)
...
def test_docker_habitat_pip_install(docker_habitat):
    ...
    # Zainstaluj pakiet
    exit_code, output = docker_habitat.execute("pip install --quiet cowsay")
    assert exit_code == 0
    
    # Sprawdź czy pakiet jest dostępny w kontenerze
    exit_code, output = docker_habitat.execute("python -c \"import cowsay; print('cowsay imported')\"")
    assert exit_code == 0
    assert "cowsay imported" in output
