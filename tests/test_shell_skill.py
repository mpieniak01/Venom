...
def test_shell_skill_initialization_sandbox():
    ...
    assert skill.use_sandbox == True or skill.use_sandbox == False  # Może być False jeśli Docker niedostępny
...
def test_shell_skill_initialization_local():
    ...
    assert skill.use_sandbox == False
...
def test_shell_skill_get_exit_code_from_error():
    ...
    output = "Komenda zakończona z błędem (exit_code=127).\n\nOutput:\nCommand not found"
    exit_code = skill.get_exit_code_from_output(output)
    
    assert exit_code == 127


def test_shell_skill_python_script_execution_sandbox(temp_workspace):
    ...
    # Utwórz skrypt
    script_path = Path(temp_workspace) / "test.py"
...
def test_shell_skill_working_directory_sandbox(temp_workspace):
    ...
    test_file = Path(temp_workspace) / "test_dir.txt"
...
def test_shell_skill_stderr_capture_sandbox(temp_workspace):
    ...
    result = skill.run_shell("python -c \"import sys; sys.stderr.write('Error output\\n')\"")
...
def test_shell_skill_stderr_capture_local(temp_workspace):
    ...
    result = skill.run_shell("python -c \"import sys; sys.stderr.write('Local error\\n')\"")
