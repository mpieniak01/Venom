...
async def test_coder_agent_initialization(mock_kernel):
    ...
    assert agent.enable_self_repair == True
...
async def test_coder_agent_initialization_without_self_repair(mock_kernel):
    ...
    assert agent.enable_self_repair == False
...
@pytest.mark.asyncio
async def test_coder_agent_process_with_verification_disabled(mock_kernel, temp_workspace):
...
@pytest.mark.asyncio
async def test_sandbox_file_visibility(temp_workspace):
...
@pytest.mark.asyncio
async def test_sandbox_pip_isolation(temp_workspace):
    ...
    assert exit_code == 0 or "Successfully installed" in result or "Requirement already satisfied" in result
    
    # Sprawdź czy pakiet jest dostępny w sandbox
    result = shell_skill.run_shell("python -c \"import requests; print('requests imported')\"")
