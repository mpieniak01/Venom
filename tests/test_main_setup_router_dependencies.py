...
def test_setup_router_dependencies_wires_globals(monkeypatch):
    ...
    monkeypatch.setattr(main_module, "system_routes", make_dummy("system"))
    ...
    assert calls["system"]["args"][0] is main_module.background_scheduler
    assert calls["system"]["args"][1] is main_module.service_monitor
