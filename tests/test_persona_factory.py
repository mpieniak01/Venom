...
def test_persona_from_dict_keeps_enum_value():
    """Gdy tech_literacy jest już enumem, from_dict powinien zadziałać bez konwersji."""
    data = {
        "name": "Ewa",
        "age": 29,
        "tech_literacy": TechLiteracy.MEDIUM,
        "patience": 0.6,
        "goal": "Sprawdzić status",
        "traits": ["dociekliwa"],
    }
    persona = Persona.from_dict(data)
    assert persona.tech_literacy == TechLiteracy.MEDIUM
...
def test_persona_from_dict_invalid_tech_literacy_raises():
    data = {
        "name": "Ewa",
        "age": 29,
        "tech_literacy": "invalid-level",
        "patience": 0.6,
        "goal": "Sprawdzić status",
        "traits": ["dociekliwa"],
    }
...
def test_persona_factory_generate_diverse_more_than_templates():
    """Dla count > liczba archetypów funkcja nadal zwraca poprawną liczbę person."""
    factory = PersonaFactory(kernel=None)
    personas = factory.generate_diverse_personas(goal="Zamówić usługę", count=9)
    assert len(personas) == 9
    assert all(p.goal == "Zamówić usługę" for p in personas)
...
def test_persona_enrichment_low_patience_fallback_branch():
    factory = PersonaFactory(kernel=None)
    persona = Persona(
        name="Marek",
        age=61,
        tech_literacy=TechLiteracy.LOW,
        patience=0.2,
        goal="Ukończyć formularz",
        traits=["ostrożny"],
        description="",
    )
    enriched = factory._enrich_persona_with_llm(persona)
    assert enriched.description
    assert "Szybko się frustruje" in enriched.description
...
def test_persona_enrichment_with_llm_short_response_falls_back():
    from unittest.mock import AsyncMock, MagicMock

    mock_kernel = MagicMock()
    mock_chat_service = MagicMock()
    mock_response = MagicMock()
    mock_response.__str__ = MagicMock(return_value="krótko")
    mock_chat_service.get_chat_message_content = AsyncMock(return_value=mock_response)
    mock_kernel.get_service = MagicMock(return_value=mock_chat_service)
    ...
    enriched = factory._enrich_persona_with_llm(persona)
    assert enriched.description
    assert "ma 34 lat" in enriched.description
    assert "krótko" not in enriched.description


def test_persona_enrichment_with_llm_exception_falls_back():
    from unittest.mock import MagicMock

    mock_kernel = MagicMock()
    mock_chat_service = MagicMock()
    mock_chat_service.get_chat_message_content = MagicMock(
    ...
    enriched = factory._enrich_persona_with_llm(persona)
    assert enriched.description
    assert "Joanna ma 40 lat" in enriched.description
...
def test_persona_enrichment_llm_fallback_on_error():
    ...
    assert (
        "60" in enriched.description or "rzadko używa komputera" in enriched.description
    )
...
def test_persona_enrichment_llm_no_name_in_response():
    ...
    assert (
        "45" in enriched.description or "podstawową znajomość" in enriched.description
    )
