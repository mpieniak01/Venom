...
- Strategia ładowania Chart.js/Mermaid/DOMPurify (czy pozostajemy przy CDN?).
- Jakie stare komponenty mogą zostać usunięte (np. `queue-tabs-panel`), a jakie trzeba porównać z nowymi (telemetry w sidebarze).

## 7. Output końcowy
- Zaktualizowany `web/templates/index.html` zgodny z `_szablon.html`.
- Uporządkowany `web/static/css/app.css` (design tokens + nowe komponenty).
- Ewentualne nowe zasoby JS/CSS (np. `web/static/css/theme_new.css`).
- Dokumentacja zmian w `docs/_to_do/042_new_ui_plan.md`, plan migracji pozostałych widoków (`docs/_to_do/043_pages_to_migrate.md`) i ewentualny wpis w changelog.
