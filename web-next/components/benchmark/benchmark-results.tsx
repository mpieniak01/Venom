...
export function BenchmarkResults({ currentResults }: BenchmarkResultsProps) {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/benchmark/list?limit=50`);
      if (res.ok) {
      ...
      <tbody className="divide-y divide-white/5">
        {results.map((r: any) => (
          <tr key={r.model_name}>
            ...
            <td className="py-2 text-right text-zinc-400">{r.tokens_per_second ? `${r.tokens_per_second} t/s` : '-'}</td>
            <td className="py-2 text-right text-zinc-400">{r.peak_vram_mb ? `${r.peak_vram_mb} MB` : '-'}</td>
            {!mini && (
