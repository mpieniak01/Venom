...
async def example_workflow_editing():
    ...
    if not workflows:
        logger.info("Brak workflow do edycji. Najpierw wygeneruj workflow używając apprentice_demo.py")
        return
    ...
    if len(workflow.steps) > 1:
        logger.info(f"📝 Wyłączanie kroku 2...")
...
""")
    ...
    logger.info("  await login_workflow(ghost, email='alice@example.com', password='pass456')")
