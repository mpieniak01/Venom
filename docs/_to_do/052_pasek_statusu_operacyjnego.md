...
## Cel
- Dodać do `web-next` kompaktową dolną belkę statusową, która stale informuje o stanie zasobów modeli oraz statusie repozytorium/wersji aplikacji – bez konieczności przewijania do panelu „Zasoby modeli” lub sekcji Git.
...
## Stan na dziś
- Panel „Zasoby modeli” (`web-next/app/page.tsx`, linie ~1140–1185) prezentuje wszystkie wymagane metryki, ale tylko w kokpicie i nie w kompaktowej formie.
...
1. **Meta-wersja w buildzie**
   - Dodać skrypt `scripts/export-version.ts`, który podczas `npm run build` zapisze do `public/meta.json` obiekt `{ version: pkg.version, commit: shortSha, timestamp }`, a także ustawi `process.env.NEXT_PUBLIC_APP_VERSION`.
   - W `LanguageProvider` lub innym globalnym miejscu udostępnić hook `useAppMeta()` czytający meta z `public/meta.json` (SSR) lub z env.
2. **Nowy komponent belki**
   - Utworzyć `SystemStatusBar` w `components/layout`, importujący `useModelsUsage`, `useGitStatus`, `useTranslation` (dla etykiet) oraz `useAppMeta`.
   - Zbudować layout: lewa sekcja = metryki (ikony + wartości), prawa sekcja = wersja + repo. Zapewnić animację wejścia (slide-up) oraz `aria-live="polite"` dla aktualizacji.
3. **Fallbacki i odświeżanie**
   - Gdy `usageMetrics` niedostępne → pokazać `Offline` + przycisk „Odśwież” wywołujący `refresh()` z hooka.
   - Gdy `git` offline → „Git API offline” + możliwość ponownego fetchu.
4. **Testy i dokumentacja**
   - Playwright: nowy scenariusz „Bottom status bar pokazuje metryki i status repo” (sprawdzi fallback `—` w trybie offline).
   - README/`FRONTEND_NEXT_GUIDE.md`: sekcja opisująca źródło danych belki.
