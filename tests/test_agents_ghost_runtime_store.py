...
def test_ghost_run_state_store_lifecycle():
    store = _make_store()
...
def test_ghost_run_state_store_invalid_json_returns_none():
...
def test_ghost_run_state_store_non_dict_payload_returns_none():
    state_path = mod._resolve_ghost_run_state_path()
    state_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_ghost_run_state_path_resolution_guards():
    default_path = (
...
def test_ghost_run_state_path_resolution_uses_tempdir_when_workspace_missing(
    ...
    monkeypatch.setattr(
        mod.SETTINGS, "WORKSPACE_ROOT", "/tmp/attacker-controlled-value"
    )
    expected = (
        Path(tempfile.gettempdir())
        / "venom"
...
@pytest.mark.asyncio
async def test_run_ghost_process_with_cancel_watch_success(monkeypatch):
    store = _make_store()
...
@pytest.mark.asyncio
async def test_run_ghost_process_with_cancel_watch_cancelled(monkeypatch):
    store = _make_store()
    monkeypatch.setattr(mod, "_ghost_run_store", store)
    mod._ghost_local_tasks.clear()
...
@pytest.mark.asyncio
async def test_run_ghost_job_updates_completed_state(monkeypatch):
    store = _make_store()
    monkeypatch.setattr(mod, "_ghost_run_store", store)
    mod._ghost_local_tasks.clear()
    store.try_start(
...
@pytest.mark.asyncio
async def test_run_ghost_job_updates_failed_state(monkeypatch):
    store = _make_store()
    monkeypatch.setattr(mod, "_ghost_run_store", store)
    mod._ghost_local_tasks.clear()
    store.try_start(
...
@pytest.mark.asyncio
async def test_run_ghost_job_updates_cancelled_state(monkeypatch):
    store = _make_store()
    monkeypatch.setattr(mod, "_ghost_run_store", store)
    mod._ghost_local_tasks.clear()
    store.try_start(
