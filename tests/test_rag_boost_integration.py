...
class TestRAGBoostIntegration:
    ...
    def test_boost_disabled_returns_baseline_for_all_intents(
    ...
    async def test_context_builder_integration_with_policy(self, mock_settings_enabled):
        ...
        with (
            patch("venom_core.core.retrieval_policy.SETTINGS", mock_settings_enabled),
            patch(
                "venom_core.core.orchestrator.task_pipeline.context_builder.SETTINGS",
    ...
    @pytest.mark.asyncio
    async def test_context_builder_no_boost_for_general_chat(
