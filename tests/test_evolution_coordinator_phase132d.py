...
class TestEvolutionCoordinatorTestExecution:
    ...
    async def test_verify_shadow_instance_fails_on_test_errors(
        ...
        assert (
            "problemy" in result["reason"].lower() or "błąd" in result["reason"].lower()
        )
    ...
    async def test_verify_shadow_instance_handles_test_timeout(
        ...
        assert (
            ...
            or "limit czasu" in result["reason"].lower()
        )
...
class TestEvolutionCoordinatorMergeAutomation:
    ...
    @pytest.mark.asyncio
    async def test_merge_changes_rollback_on_unmerged_blobs(
        self, coordinator, mock_git_skill
    ):
