...
def validate_task_type():
    ...
    assert 'RESEARCH = "RESEARCH"' in code, \
        "❌ Missing RESEARCH task type"
    print("✓ TaskType.RESEARCH exists")
    
    # Check for comment
    assert '# Badania, wyszukiwanie w Internecie' in code or 'RESEARCH' in code, \
...
def validate_router_logic():
    ...
    assert 'if task_type == TaskType.RESEARCH:' in code, \
        "❌ Missing RESEARCH routing logic"
    print("✓ RESEARCH routing logic exists")
    
    # Check for paid_mode comment or reference
    assert 'RESEARCH' in code and ('Google' in code or 'DuckDuckGo' in code), \
...
def validate_researcher_agent():
    ...
    assert 'def format_grounding_sources' in code, \
    ...
    assert 'grounding_metadata' in code, \
        "❌ Missing grounding_metadata handling"
    print("✓ grounding_metadata handling exists")
    
    # Check for get_last_search_source method
    assert 'def get_last_search_source' in code, \
        "❌ Missing get_last_search_source method"
    print("✓ get_last_search_source method exists")
    
    # Check for _last_search_source field
    assert '_last_search_source' in code, \
        "❌ Missing _last_search_source field"
    print("✓ _last_search_source field exists")
    
    # Check for sources section formatting
    assert '📚 Źródła (Google Grounding)' in code, \
        "❌ Missing sources section formatting"
...
def validate_frontend():
    ...
    assert 'research-source-badge' in code, \
        "❌ Missing research-source-badge class"
    print("✓ research-source-badge class exists")
    
    # Check for google_grounding badge
    assert 'google_grounding' in code, \
        "❌ Missing google_grounding badge"
    print("✓ google_grounding badge exists")
    
    # Check for duckduckgo badge
    assert 'duckduckgo' in code, \
        "❌ Missing duckduckgo badge"
...
def validate_tests():
    ...
    assert 'test_route_research_task' in code, \
        "❌ Missing test_route_research_task tests"
    ...
    assert 'test_kernel_builder_enable_grounding_parameter' in code, \
...
def validate_documentation():
    ...
    assert '# Google Search Grounding Integration' in content, \
        "❌ Missing main title"
    print("✓ Main title exists")
    
    assert 'Architektura' in content or 'Architecture' in content, \
        "❌ Missing architecture section"
    print("✓ Architecture section exists")
    
    assert 'Kryteria Akceptacji' in content or 'DoD' in content, \
        "❌ Missing acceptance criteria section"
    print("✓ Acceptance criteria section exists")
    
    assert 'paid_mode' in content, \
        "❌ Missing paid_mode documentation"
    print("✓ paid_mode documentation exists")
    
    assert 'Google Grounding' in content, \
        "❌ Missing Google Grounding documentation"
