...
class IntentEmbeddingRouter:
    ...
    def __init__(
        ...
        self.min_score = min_score if min_score is not None else SETTINGS.INTENT_EMBED_MIN_SCORE
    ...
    def _initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                "Błąd inicjalizacji Intent Embedding Router: %s",
                exc,
                exc_info=True
            )
            raise
    ...
    def _load_intent_phrases(self) -> None:
        ...
        logger.debug(
            "Załadowano frazy dla intencji: %s",
            list(self.intent_phrases.keys())
        )
    ...
    def _build_intent_embeddings(self) -> None:
        ...
        for intent_name, phrases in self.intent_phrases.items():
            ...
            try:
                ...
                logger.warning(
                    "Błąd tworzenia embeddingu dla intencji %s: %s",
                    intent_name,
                    exc
                )
