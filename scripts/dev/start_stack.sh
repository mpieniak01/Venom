...
NEXT_DEV_ENV="${NEXT_DEV_ENV:-NEXT_MODE=dev NEXT_DISABLE_TURBOPACK=1 NEXT_TELEMETRY_DISABLED=1}"
NEXT_PROD_ENV="${NEXT_PROD_ENV:-NEXT_MODE=prod NEXT_TELEMETRY_DISABLED=1}"
NEXT_TURBO_WATCH_ENV="${NEXT_TURBO_WATCH_ENV:-WATCHPACK_POLLING=true WATCHPACK_POLLING_INTERVAL=1000 CHOKIDAR_USEPOLLING=1}"
...
if [[ -z "$ui_skip" ]]; then
  ...
  if [[ "$START_MODE" == "prod" ]]; then
    ...
    if [[ "$START_WEB_MODE" == "turbo" ]]; then
      ...
      (
        cd "$WEB_DIR"
        NODE_PATH="$WEB_NODE_PATH" NEXT_PUBLIC_APP_VERSION="$WEB_APP_VERSION" NEXT_PUBLIC_ENVIRONMENT_ROLE="${ENVIRONMENT_ROLE:-dev}" NEXT_MODE=dev WATCHPACK_POLLING=true WATCHPACK_POLLING_INTERVAL=1000 CHOKIDAR_USEPOLLING=1 "$PYTHON_BIN" -m dotenv -f "$ENV_FILE" run -- setsid "$NPM" run dev:turbo:debug -- --hostname "$WEB_HOST" --port "$WEB_PORT" >> "../$WEB_LOG" 2>&1 &
        echo $! > "../$WEB_PID_FILE"
      )
    else
