...
def demo_state_manager():
    ...
    if state_manager.is_paid_mode_enabled():
        raise AssertionError("Domyślnie paid_mode powinien być wyłączony")
...
Market cap wynosi około $850 miliardów [2]."""
    ...
    if empty_sources != "":
        raise AssertionError("Puste metadane powinny zwracać pusty string")
...
def demo_acceptance_criteria():
    ...
    print("   Expected: LOCAL (DuckDuckGo)")
    ...
    print(f"   paid_mode: {state_manager.is_paid_mode_enabled()}")
    print(f"   Routing target: {routing['target']}")
    print("   Note: Faktyczne użycie Google wymaga GOOGLE_API_KEY")
    ...
    metadata = {
        "grounding_metadata": {
            "grounding_chunks": [{"title": "Test", "uri": "https://example.com"}]
        }
    }
