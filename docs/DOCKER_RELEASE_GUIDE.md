...
- For tag mode: first make sure commit is on `main`, then create and push tag `v1.9.0` (current stable). Pushing the tag starts publish automatically. The workflow rejects `v1.9` because it requires full semver.
...
2. Stable release uses semantic version tag: `vMAJOR.MINOR.PATCH` (current example: `v1.9.0`).
   - `v1.9` will fail preflight.
...
```bash
docker pull ghcr.io/mpieniak01/venom-backend:v1.9.0
docker pull ghcr.io/mpieniak01/venom-frontend:v1.9.0
```
...
# optional overrides:
# export BACKEND_IMAGE=ghcr.io/mpieniak01/venom-backend:v1.9.0
# export FRONTEND_IMAGE=ghcr.io/mpieniak01/venom-frontend:v1.9.0
...
### Protocol policy (no manual tuning in default path)

- Local/dev Docker stack (`run-release.sh`, `compose.release.yml`, `compose.minimal.yml`) defaults to `URL_SCHEME_POLICY=force_http`.
- Internal service links stay HTTP (`frontend -> backend`, `backend -> ollama`) to avoid accidental TLS mismatch inside container network.
- Public production exposure should terminate TLS on reverse proxy/ingress (HTTPS on edge, HTTP inside stack unless explicitly hardened end-to-end).
