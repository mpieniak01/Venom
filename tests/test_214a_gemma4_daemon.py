...
def test_respond_release_after_response_skips_cleanup_when_inflight(
    monkeypatch,
) -> None:
    daemon = _make_daemon()
    unload_calls = []
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["post_response_cleanup"] == "cleanup_skipped_inflight"
    assert unload_calls == []


def test_respond_release_after_response_handles_cleanup_error(monkeypatch) -> None:
    daemon = _make_daemon()

    def _unload_all():
        raise RuntimeError("gpu cleanup failure")
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["post_response_cleanup"] == "cleanup_failed"
