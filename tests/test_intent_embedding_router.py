...
def mock_sentence_transformer():
    ...
    def mock_encode(texts, convert_to_numpy=True, show_progress_bar=False):
        ...
        for text in texts:
            ...
            if any(
                word in text_lower
                for word in ["write", "function", "code", "script", "napisz", "kod"]
            ):
                # Embedding dla CODE_GENERATION
                emb = np.array([1.0, 0.0, 0.0])
            elif any(
                word in text_lower for word in ["what", "explain", "co to", "wyjaśnij"]
            ):
...
class TestIntentEmbeddingRouter:
            ...
            with patch.object(SETTINGS, "INTENT_EMBED_MIN_SCORE", 0.5):
                with patch.object(SETTINGS, "INTENT_EMBED_MARGIN", 0.1):
                    with patch(
                        ...
                        router = IntentEmbeddingRouter(temp_lexicon_dir)
                        intent, score, top2 = await router.classify(
    ...
    async def test_classify_below_threshold(
        ...
        with patch.object(SETTINGS, "ENABLE_INTENT_EMBEDDING_ROUTER", True):
            with patch.object(
                SETTINGS, "INTENT_EMBED_MIN_SCORE", 0.99
            ):  # Bardzo wysoki próg
    ...
    async def test_classify_insufficient_margin(
        ...
        with patch.object(SETTINGS, "ENABLE_INTENT_EMBEDDING_ROUTER", True):
            with patch.object(SETTINGS, "INTENT_EMBED_MIN_SCORE", 0.5):
                with patch.object(
                    SETTINGS, "INTENT_EMBED_MARGIN", 0.3
                ):  # Wysoki wymagany margines
                    with patch(
                        "sentence_transformers.SentenceTransformer",
...
@pytest.mark.asyncio
async def test_intent_manager_with_embedding_router(
    temp_lexicon_dir, mock_sentence_transformer
):
        ...
        with patch.object(SETTINGS, "INTENT_EMBED_MIN_SCORE", 0.5):
            with patch.object(SETTINGS, "INTENT_EMBED_MARGIN", 0.1):
                with patch(
                    ...
                    with patch(
                        "venom_core.core.intent_manager.IntentManager.LEXICON_DIR",
                        temp_lexicon_dir,
                    ):
